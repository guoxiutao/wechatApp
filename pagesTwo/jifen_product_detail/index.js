  ...
  buyGoods: function (e) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          console.log('用户点击确定')
          that.exchange(that.data.jifenId)
        } else if (res.cancel) {
          console.log('用户点击取消')
        }
      }
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.errcode == "-1") {
            ...
            // 兑换成功后次数减少
            that.lookMore(res.data.relateObj.orderNo)
          }
  ...
  onLoad: function (options) {
    ...
    if (options.type=='jifen'){
      that.setData({ 
        ...
        jifenNum: options.jifenNum || 0,
        jifenCount: options.jifenCount || 0,
        jifenId: options.jifenId || 0
      })
    }
    console.log('--------buyType----------', that.data.buyType)
    that.setData({
