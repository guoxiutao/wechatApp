...
Page({
  ...
  getData: function (param, ifAdd) {
    ...
    var customIndex = app.AddClientUrl("/wx_find_asset_space_organizes.html", param)
    wx.showLoading({
      title: 'loading'
    })
    var that = this
    wx.request({
      ...
      success: function (res) {
        wx.hideLoading()
        console.log("===organizes=====",res.data)
        ...
        let dataArr = that.data.organizeList
        let tagArray=[];
        if (ifAdd == 2) {
          dataArr = []
        }
        if (!res.data.relateObj.result || res.data.relateObj.result.length == 0) {
          that.setData({ organizeList: null })
        } else {
          ...
          that.setData({ organizeList: dataArr })
        }
        that.setData({ markers: that.data.organizeList })
        let conut=0;
